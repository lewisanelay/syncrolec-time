...
import { ArrowLeft, ChevronLeft, ChevronRight, Download, Delete, Check, X } from "lucide-react";
...
type Props = { employeeId: number; employeeName: string; onBack: () => void };

export default function MyHoursPage({ employeeId, employeeName, onBack }: Props) {
  ...
  const { data: monthShifts = [] } = useQuery<Shift[]>({
    queryKey: ["/api/shifts/month", monthStr, employeeId],
    queryFn: () =>
      apiRequest("GET", `/api/shifts/month?month=${monthStr}&employeeId=${employeeId}`)
        .then(r => r.json()),
  });
  ...
  const exportCSV = () => {
    const rows = [
      ["Date", "Clock In", "Clock Out", "Regular Hours", "Overtime Hours", "Pay (£)"],
      ...monthShifts
    ...
    a.href = url;
    a.download = `${employeeName.replace(" ", "-")}-hours-${monthStr}.csv`;
    a.click();
...
<html><head><title>${employeeName} — Hours ${monthLabel}</title>
<style>
...
<body>
  <h1>SYNCROLEC — My Hours</h1>
  <div class="sub">${employeeName} &nbsp;·&nbsp; ${monthLabel}</div>
  <table>
    <thead><tr>
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-background z-10">
        <div className="flex items-center gap-3">
          <button
            ...
            data-testid="button-my-hours-back"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            <div className="text-sm font-bold text-foreground tracking-wider">SYNCROLEC</div>
            <div className="text-xs text-muted-foreground">My Hours — {employeeName}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            data-testid="button-my-hours-csv"
          >
          ...
          <button
            ...
            data-testid="button-my-hours-pdf"
          >
      ...
      <div className="px-4 py-5 max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-center gap-4">
          <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-secondary transition-colors" data-testid="button-my-prev-month">
            <ChevronLeft size={18} className="text-muted-foreground" />
          </button>
          <span className="text-base font-semibold text-foreground min-w-[140px] text-center">{monthLabel}</span>
          <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-secondary transition-colors" data-testid="button-my-next-month">
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isSelected ? "bg-primary/15" : isValid ? "hover:bg-secondary/60" : ""}
                    transition-colors
                  `}
                >
                  {isValid && (
                    <>
                      ...
                      {dayShifts.length > 0 && (
                        <div className="flex flex-col gap-0.5">
                          ...
                          {hasOT && <div className="text-[9px] text-yellow-400/80">OT</div>}
                        </div>
                      )}
                    </>
                  )}
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            ...
            {selectedShifts.length === 0 ? (
              ...
              selectedShifts.map(shift => (
                <div key={shift.id} className="flex items-center justify-between py-2 border-t border-border first:border-0">
                  <div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {formatTime(shift.clockInTime)} → {shift.clockOutTime ? formatTime(shift.clockOutTime) : "still clocked in"}
                    </div>
                    {(shift.overtimeHours || 0) > 0 && (
                      <div className="text-xs text-yellow-400 mt-0.5">{formatHours(shift.overtimeHours)} overtime</div>
                    )}
                  </div>
            ...
            {selectedShifts.some(s => s.clockOutTime) && (
        ...
        <p className="text-xs text-muted-foreground text-center pb-2">
          Tap any day to see details · Green = hours worked · Yellow = overtime
        </p>
      </div>
    </div>
  );
